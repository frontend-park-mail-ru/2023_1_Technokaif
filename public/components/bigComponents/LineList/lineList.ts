...
export class LineList extends BaseComponent {
    ...
    /**
     * Function to rerender Tracks
     * @private
     */
    private reRenderTracks() {
        UserActions.favoriteTracks(localStorage.getItem('userId'));
    }

    /**
     * Function for lineList callback
     * @param event
     * @private
     */
    private lineListCallback(event) {
        ...
        const another: HTMLDivElement | null = event.target.closest(`.${this.elementConfig.anotherBlock}`);
        ...
        if (line) {
            ...
            if (event.target === buttons) {
                ...
                const index: number = Array.from(spansInArtists as NodeListOf<HTMLSpanElement>).indexOf(artist);
                if (index !== -1) {
    ...
    #renderDropDownForOneLine(line: HTMLElement, index, another) {
        ...
        const anotherImg: Element|undefined = document.querySelectorAll(`.${this.elementConfig.anotherClass}`)[index];
        if (!anotherImg) {
            console.error('Not found image on linelist');
            return;
        }
        anotherImg.addEventListener('click', () => { another.click(); });
    ...
    #addListeners() {
        ContentStore.subscribe((instance) => {
            if (this.config.playlistId || this.elementConfig.isPlusTrack) return;
            ...
            const anothers = document.querySelectorAll(`.${this.elementConfig.anotherBlock}`);
            ...
            lines.forEach((line, index) => {
                ...
                const div = document.createElement('div');
                div.classList.add(`${this.elementConfig.anotherBlock}`);
                ...
                if (!element || !(parentOfDots) || !(element instanceof HTMLDivElement)) {
        ...
        this.parent.removeEventListener('click', this.lineListCallback.bind(this));
        this.parent.addEventListener('click', this.lineListCallback.bind(this));
        ...
        API.subscribe(
            (message, id, playlistId) => {
                if (message === 'OK' && this.name === componentsNames.PLAYLIST) {
                    this.unrenderTrack(id);
                    ContentStore.state[pageNames.LIBRARY_PLAYLISTS].playlists = ContentStore.state[pageNames.LIBRARY_PLAYLISTS].playlists.map((playlistOne) => {
                        if (playlistOne.id === Number(playlistId)) {
                            playlistOne.tracks = playlistOne.tracks.filter((track) => track.id !== Number(id));
                        }
        ...
        if (this.config.playlistId) {
            const anothers = document.querySelectorAll(`.${this.elementConfig.anotherBlock}`) as NodeListOf<HTMLDivElement>;
